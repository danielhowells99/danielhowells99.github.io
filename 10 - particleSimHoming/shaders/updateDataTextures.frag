...
void main() {
	...
	vec2 homePos = homeData.xy;
	
	position *= transformVector;
	homePos *= transformVector;

	vec2 velocity = data.zw; //v.xy 2d

	vec2 homeDisplacement = homePos - position;
	float homeDist = length(homeDisplacement);
	...
	*/
	...
	velocity = pow(0.97,30.0*uDeltaTime)*velocity + uDeltaTime*force;//SETTING2
