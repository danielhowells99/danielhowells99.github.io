...
vec2 wedge1(vec2 centre,vec2 coordIncrement,vec2 flips){ //0 - 2pi/8
	...
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(5.0,0.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(6.0,0.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(7.0,0.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(8.0,0.0)).w;
	state = updateMeanAndSd(x_i,state);
	...
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(4.0,1.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(5.0,1.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(6.0,1.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(7.0,1.0)).w;
	state = updateMeanAndSd(x_i,state);
	...
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(4.0,2.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(5.0,2.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(6.0,2.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(7.0,2.0)).w;
	state = updateMeanAndSd(x_i,state);

	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(3.0,3.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(4.0,3.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(5.0,3.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(6.0,3.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(7.0,3.0)).w;
	state = updateMeanAndSd(x_i,state);

	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(4.0,4.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(5.0,4.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(6.0,4.0)).w;
	state = updateMeanAndSd(x_i,state);

	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(5.0,5.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(6.0,5.0)).w;
	state = updateMeanAndSd(x_i,state);
...
vec2 wedge2(vec2 centre,vec2 coordIncrement,vec2 flips){ //0 - 2pi/8
	...
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(0.0,5.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(0.0,6.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(0.0,7.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(0.0,8.0)).w;
	state = updateMeanAndSd(x_i,state);
	...
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(1.0,4.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(1.0,5.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(1.0,6.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(1.0,7.0)).w;
	state = updateMeanAndSd(x_i,state);
	...
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(2.0,4.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(2.0,5.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(2.0,6.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(2.0,7.0)).w;
	state = updateMeanAndSd(x_i,state);

	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(3.0,3.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(3.0,4.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(3.0,5.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(3.0,6.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(3.0,7.0)).w;
	state = updateMeanAndSd(x_i,state);

	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(4.0,4.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(4.0,5.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(4.0,6.0)).w;
	state = updateMeanAndSd(x_i,state);

	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(5.0,5.0)).w;
	state = updateMeanAndSd(x_i,state);
	x_i = texture2D(uFbTexture,centre + flips*coordIncrement*vec2(5.0,6.0)).w;
	state = updateMeanAndSd(x_i,state);
