...
void main() {
  ...
  vec3 distortion = sin(positionVec4.xyz * frequency + 0.1*uFrameCount + vec3(0.01,0.02,0.03));
  positionVec4.xyz += distortion * aNormal.xyz * amplitude;

  // Send the normal to the fragment shader
  vNormal = aNormal;
  absolute = positionVec4.xyz;
